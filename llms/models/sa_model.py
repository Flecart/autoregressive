...
class SAGPTBlock(karpathy.GPTBlock):
    def __init__(self, config: configs.GPTConfig):
        super().__init__(n_embd=config.n_embd, n_head=config.n_head, n_inner=config.n_inner, dropout=config.dropout, bias=config.bias)
...
class SAGPT(karpathy.MetaGPT):
    ...
    def forward(self, idx, targets=None):
        ...
        if targets is not None:
            ...
            logits = torch.stack([self.lm_heads[i](x)[:, i::self.config.k_regressivity] for i in range(self.config.k_regressivity)]) # (k, b, t, vocab_size)
            stacked_targets = torch.stack([targets[:, i::self.config.k_regressivity] for i in range(self.config.k_regressivity)], ) # (k, b, t)
            # logits = self.lm_head(x) # (b, t, vocab_size)
            loss: torch.Tensor = F.cross_entropy(logits.view(-1, logits.size(-1)), stacked_targets.view(-1), reduction="none") * masks.view(-1)
            loss = loss.mean()
        else:
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, top_k=None, stop=None):
        ...
        """
        for _ in range(0, max_new_tokens, self.config.k_regressivity):
